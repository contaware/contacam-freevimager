  ...
  for (ci = 0; ci < dstinfo->num_components; ci++) {
    ...
    for (dst_blk_y = 0; dst_blk_y < compptr->height_in_blocks;
	 dst_blk_y += compptr->v_samp_factor) {
		DIB_PROGRESS(hWnd, bProgressSend, ci * compptr->height_in_blocks + dst_blk_y,
		dstinfo->num_components * compptr->height_in_blocks);
  ...
  for (ci = 0; ci < dstinfo->num_components; ci++) {
    ...
    for (blk_y = 0; blk_y < compptr->height_in_blocks;
	 blk_y += compptr->v_samp_factor) {
		DIB_PROGRESS(hWnd, bProgressSend, ci * compptr->height_in_blocks + blk_y,
		dstinfo->num_components * compptr->height_in_blocks);
  ...
  case JXFORM_NONE:
    if (info->x_crop_offset != 0 || info->y_crop_offset != 0)
      do_crop(srcinfo, dstinfo, info->x_crop_offset, info->y_crop_offset,
	      src_coef_arrays, dst_coef_arrays, hWnd, bProgressSend);
    break;
  case JXFORM_FLIP_H:
    if (info->y_crop_offset != 0)
      ...
      do_flip_h_no_crop(srcinfo, dstinfo, info->x_crop_offset,
			src_coef_arrays, hWnd, bProgressSend);
    break;
  case JXFORM_FLIP_V:
...
/* Copy markers saved in the given source object to the destination object,
 * replacing first found marker, or creating it if not existing.
 * This should be called just after jpeg_start_compress() or
 * jpeg_write_coefficients().
 * Note that those routines will have written the SOI, and also the
 * JFIF APP0 or Adobe APP14 markers if selected.
...
GLOBAL(void)
jcopy_markers_execute_addmarker (j_decompress_ptr srcinfo, j_compress_ptr dstinfo,
				JOCTET data_marker, JOCTET * data_ptr, INT32 data_size, JCOPY_OPTION option)
{
	int first = 1;
	jpeg_saved_marker_ptr marker;
	...
	*/
	for (marker = srcinfo->marker_list ; marker != NULL ; marker = marker->next)
	{
		...
		if (dstinfo->write_Adobe_marker &&
			...
			GETJOCTET(marker->data[4]) == 0x65)
			continue;			/* reject duplicate Adobe */
		if ((marker->marker == data_marker) && first) /* replace marker */
		{
			first = 0;
#ifdef NEED_FAR_POINTERS
			/* We could use jpeg_write_marker if the data weren't FAR... */
			{
				unsigned int i;
				jpeg_write_m_header(dstinfo, marker->marker, data_size);
				for (i = 0 ; i < data_size ; i++)
					jpeg_write_m_byte(dstinfo, data_ptr[i]);
			}
#else
			jpeg_write_marker(dstinfo, marker->marker,
							data_ptr, data_size);
	...
	if (first)
	{
		...
		{
			unsigned int i;
			jpeg_write_m_header(dstinfo, data_marker, data_size);
			for (i = 0 ; i < data_size ; i++)
				jpeg_write_m_byte(dstinfo, data_ptr[i]);
		}
#else
		jpeg_write_marker(dstinfo, data_marker,
						data_ptr, data_size);
#endif
	}
}
