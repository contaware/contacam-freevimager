...
HRESULT GetDXVersion(DWORD* pdwDirectXVersion, TCHAR* strDirectXVersion, int cchDirectXVersion)
{
    ...
    cDirectXVersionLetter = _totlower(cDirectXVersionLetter);
    ...
    if( pdwDirectXVersion )
    {
        ...
        dwDirectXVersion <<= 8;
        if( cDirectXVersionLetter >= _T('a') && cDirectXVersionLetter <= _T('z') )
            dwDirectXVersion += (cDirectXVersionLetter - _T('a')) + 1;
    ...
    if( strDirectXVersion && cchDirectXVersion > 0 )
    {
        ...
        if( cDirectXVersionLetter == _T(' ') )
            _sntprintf( strDirectXVersion, cchDirectXVersion, _T("%d.%d"), dwDirectXVersionMajor, dwDirectXVersionMinor );
        else
            _sntprintf( strDirectXVersion, cchDirectXVersion, _T("%d.%d%c"), dwDirectXVersionMajor, dwDirectXVersionMinor, cDirectXVersionLetter );
        strDirectXVersion[cchDirectXVersion-1] = _T('\0');
    }
                                   ...
                                   TCHAR* pcDirectXVersionLetter)
{
	...
	bCleanupCOM = ((hr == S_OK) || (hr == S_FALSE));
