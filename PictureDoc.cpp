...
BEGIN_MESSAGE_MAP(CPictureDoc, CUImagerDoc)
	...
	ON_COMMAND(ID_EDIT_FILTER_EDGE, OnEditFilterEdge)
	ON_UPDATE_COMMAND_UI(ID_EDIT_FILTER_EDGE, OnUpdateEditFilterEdge)
	ON_COMMAND(ID_EDIT_FILTER_EMBOSS, OnEditFilterEmboss)
	ON_UPDATE_COMMAND_UI(ID_EDIT_FILTER_EMBOSS, OnUpdateEditFilterEmboss)
	ON_COMMAND(ID_EDIT_FILTER_BLUR, OnEditFilterBlur)
	ON_UPDATE_COMMAND_UI(ID_EDIT_FILTER_BLUR, OnUpdateEditFilterBlur)
	ON_COMMAND(ID_EDIT_FILTER_GAUSSIAN3X3, OnEditFilterGaussian3x3)
	ON_UPDATE_COMMAND_UI(ID_EDIT_FILTER_GAUSSIAN3X3, OnUpdateEditFilterGaussian3x3)
	ON_COMMAND(ID_EDIT_FILTER_GAUSSIAN5X5, OnEditFilterGaussian5x5)
	ON_UPDATE_COMMAND_UI(ID_EDIT_FILTER_GAUSSIAN5X5, OnUpdateEditFilterGaussian5x5)
...
BOOL CPictureDoc::CLayeredDlgThread::DoIt(CWorkerThread* pThread/*=NULL*/) 
{
	...
	// We need some transparent top borders,
	// otherwise we see the rounded window corners!
	if (!m_Dib.AddBorders(	0,
...
BOOL CPictureDoc::IsJPEG(LPCTSTR szFileName)
{
	return ((::GetFileExt(szFileName) == _T(".jpg"))	||
...
BOOL CPictureDoc::SaveAsPdf(BOOL bShowPdfSaveDlg)
{
	...
	if (dlgFile.DoModal() == IDOK)
	{
		...
		if (res)
		{
			...
			// always lossless (jpeg is not recompressed):
			//
...
BOOL CPictureDoc::EditLinearFilter(BOOL bShowMessageBoxOnError, int nTypeID) 
{
	if (m_pDib)
	{
		// Wait and schedule command if dib not fully loaded!
		...
		if (nTypeID == ID_EDIT_FILTER_SHARPEN)
		{
			int Kernel[] = {-1,-1,-1,
							-1,20,-1,
							-1,-1,-1};
			res = m_pDib->FilterFast(Kernel, 12,
									NULL,
									GetView(),
									TRUE);
		}
		...
		else if (nTypeID == ID_EDIT_FILTER_SOFTEN)
		{
			int Kernel[] = {1,1,1,
							1,8,1,
							1,1,1};
			res = m_pDib->FilterFast(Kernel, 16,
									NULL,
									GetView(),
									TRUE);
		}
		...
		else if (nTypeID == ID_EDIT_FILTER_EDGE)
		{
			int Kernel[] = {-1,-1,-1,
							-1, 8,-1,
							-1,-1,-1};
			res = m_pDib->Filter(Kernel, 3, -1, 255,
								NULL,
								GetView(),
								TRUE);
		}
		...
		else if (nTypeID == ID_EDIT_FILTER_EMBOSS)
		{
			int Kernel[] = {0,0,-1,
							0,0,0,
							1,0,0};
			res = m_pDib->Filter(Kernel, 3, 0, 127,
								NULL,
								GetView(),
								TRUE);
		}
		...
		else if (nTypeID == ID_EDIT_FILTER_BLUR)
		{
			int Kernel[] = {1,1,1,
							1,1,1,
							1,1,1};
			res = m_pDib->FilterFast(Kernel, 9,
									NULL,
									GetView(),
									TRUE);
		}
		// Gaussian 3x3, the gaussian filter removes high
		// frequency noise, but causes blurring. The larger
		// the kernel, the greater the defocus.
		...
		else if (nTypeID == ID_EDIT_FILTER_GAUSSIAN5X5)
		{
			int Kernel[] = {0,1,2,1,0,
							...
							1,3,4,3,1,
							0,1,2,1,0};
			res = m_pDib->Filter(Kernel, 5, 52, 0,
								NULL,
								GetView(),
								TRUE);
		}
...
void CPictureDoc::OnEditFilterEdge() 
{
	EditLinearFilter(TRUE, ID_EDIT_FILTER_EDGE);
}

void CPictureDoc::OnUpdateEditFilterEdge(CCmdUI* pCmdUI) 
{
	pCmdUI->Enable(	(m_dwIDAfterFullLoadCommand == 0 ||
					m_dwIDAfterFullLoadCommand == ID_EDIT_FILTER_EDGE) &&
					DoEnableCommand());
}

void CPictureDoc::OnEditFilterEmboss() 
{
	EditLinearFilter(TRUE, ID_EDIT_FILTER_EMBOSS);
}

void CPictureDoc::OnUpdateEditFilterEmboss(CCmdUI* pCmdUI) 
{
	pCmdUI->Enable(	(m_dwIDAfterFullLoadCommand == 0 ||
					m_dwIDAfterFullLoadCommand == ID_EDIT_FILTER_EMBOSS) &&
					DoEnableCommand());
}

void CPictureDoc::OnEditFilterBlur() 
{
	EditLinearFilter(TRUE, ID_EDIT_FILTER_BLUR);
}

void CPictureDoc::OnUpdateEditFilterBlur(CCmdUI* pCmdUI) 
{
	pCmdUI->Enable(	(m_dwIDAfterFullLoadCommand == 0 ||
					m_dwIDAfterFullLoadCommand == ID_EDIT_FILTER_BLUR) &&
					DoEnableCommand());
}

void CPictureDoc::OnEditFilterGaussian3x3() 
{
	EditLinearFilter(TRUE, ID_EDIT_FILTER_GAUSSIAN3X3);
}

void CPictureDoc::OnUpdateEditFilterGaussian3x3(CCmdUI* pCmdUI) 
{
	pCmdUI->Enable(	(m_dwIDAfterFullLoadCommand == 0 ||
					m_dwIDAfterFullLoadCommand == ID_EDIT_FILTER_GAUSSIAN3X3) &&
					DoEnableCommand());
}

void CPictureDoc::OnEditFilterGaussian5x5() 
{
	EditLinearFilter(TRUE, ID_EDIT_FILTER_GAUSSIAN5X5);
}

void CPictureDoc::OnUpdateEditFilterGaussian5x5(CCmdUI* pCmdUI) 
{
	pCmdUI->Enable(	(m_dwIDAfterFullLoadCommand == 0 ||
					m_dwIDAfterFullLoadCommand == ID_EDIT_FILTER_GAUSSIAN5X5) &&
					DoEnableCommand());
}
...
BOOL CPictureDoc::UpdateLayeredDlg(CDib* pDib)
{
	...
	else
	{
		...
		if (m_bFirstLayeredDlgUpdate)
		{
			...
			CDib EmptyDib;
			EmptyDib.AllocateBits(32, BI_RGB, m_nLayeredDlgWidth, m_nLayeredDlgHeight - LAYERED_DLG_TOPBORDER, RGBA(0,0,0,255));
			EmptyDib.SetAlpha(TRUE);
			EmptyDib.AddBorders(0,
								LAYERED_DLG_TOPBORDER,
								0,
								0,
								0);
