...
BOOL CHLSDlgModeless::DoItBigPicture()
{
	BOOL res;
	CPictureView* pView = (CPictureView*)m_pParentWnd;
	CPictureDoc* pDoc = (CPictureDoc*)pView->GetDocument();
	...
	CString sOrigFileName = pDoc->m_sFileName;

	CString sHLSFileName;
	int nID;
	if (pDoc->m_pDib->IsMMReadOnly())
		nID = IDYES;
	else
		nID = ::AfxMessageBox(ML_STRING(1389, "Do You Want To Save The Color Adjusted Image To A New File?"), MB_YESNOCANCEL);
	if (nID == IDYES)
	{
		// Display the Save As Dialog
		TCHAR szFileName[MAX_PATH];
		CNoVistaFileDlg dlgFile(FALSE);
		sHLSFileName = sOrigFileName;
		int index = sHLSFileName.ReverseFind(_T('.'));	
		if (index > 0)
		{
			CString s(_T("_coloradjust"));
			sHLSFileName.Insert(index, s);
		}
		else
			goto error;
		_tcscpy(szFileName, sHLSFileName);
		...
		{
			sHLSFileName = szFileName;
			if (pDoc->m_pDib->IsMMReadOnly() && sHLSFileName == pDoc->m_sFileName)
			{
	...
	if (nID == IDYES)
	{
		...
		if (!::CopyFile(sOrigFileName, sHLSFileName, FALSE))
		{
			int nLastError = ::GetLastError();
			EndWaitCursor();
			::ShowError(nLastError, pDoc->IsShowMessageBoxOnError());
			BeginWaitCursor();
			pDoc->LoadBigPicture(sOrigFileName); // Restore
			EndWaitCursor();
			goto error;
		}

		// Make Sure the Copy is Not Read Only!
		DWORD dwAttrib = ::GetFileAttributes(sHLSFileName);
		if ((dwAttrib != 0xFFFFFFFF) && (dwAttrib & FILE_ATTRIBUTE_READONLY))
			::SetFileAttributes(sHLSFileName, dwAttrib & ~FILE_ATTRIBUTE_READONLY);

		// Load the Copy
		if (!pDoc->LoadBigPicture(sHLSFileName))
		{
			pDoc->LoadBigPicture(sOrigFileName); // Restore
			EndWaitCursor();
			goto error;
	...
	// Adjust Color
	AdjustColor(TRUE); // Of Preview
	res = AdjustColor(FALSE); // Of Big Picture
	...
	{		
		...
		CString str;
		str = ML_STRING(1418, "Error while adjusting the color of the big picture\n");
		TRACE(str);
		...
		{
			BeginWaitCursor();
			pDoc->LoadBigPicture(sOrigFileName);
			EndWaitCursor();
		}
