							...
							int nThreadCount)
{
	...
	if (pCodecCtx->codec_id == AV_CODEC_ID_H264)
	{
		...
		// For libx264 compiled with gcc disable 256-bit and 512-bit instructions because mingw doesn't
		// support 32-bytes and 64-bytes stack alignment on Windows
		// Attention: remember to disable newer SIMD instructions when recompiling libx264 (see x264.h)
		// Note:      to make sure the following command works, enable most verbose logging and search
		//            for the "using cpu capabilities" message generated by avcodec_open2()
#if !defined(FFMPEG_TOOLCHAIN_MSVC) || (FFMPEG_TOOLCHAIN_MSVC != 1)
		...
		cpu &= ~(X264_CPU_LZCNT | X264_CPU_AVX | X264_CPU_XOP | X264_CPU_FMA4 | X264_CPU_FMA3 | X264_CPU_BMI1 | X264_CPU_BMI2 | X264_CPU_AVX2 | X264_CPU_AVX512);
		CStringA sAsmOption;
