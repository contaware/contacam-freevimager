 ...
 *****************************************************************************/
GifFileType *
DGifOpenFileName(LPCTSTR FileName) {
    int FileHandle;
    GifFileType *GifFile;

    if ((FileHandle = _topen(FileName, O_RDONLY
 ...
 *****************************************************************************/
GifFileType *
DGifOpenFileHandle(int* pFileHandle) {
...
#if defined(__MSDOS__) || defined(_OPEN_BINARY)
    setmode(*pFileHandle, O_BINARY);    /* Make sure it is in binary mode. */
#endif /* __MSDOS__ || _OPEN_BINARY */

    f = fdopen(*pFileHandle, "rb");    /* Make it into a stream: */
    ...
    Private->FileHandle = *pFileHandle;
    Private->File = f;
    ...
    if (READ(GifFile, Buf, GIF_STAMP_LEN) != GIF_STAMP_LEN) {
        ...
        fclose(f);
		*pFileHandle = -1; // Do not close two times!
        free((char *)Private);
        free((char *)GifFile);
        return NULL;
    }
