...
TCHAR g_s3Levels[2048];

void DisplayCpuCount(void)
{
	CString s, t;
	...
	switch (StatusFlag)
	{
		case MULTI_CORE_AND_HT_NOT_CAPABLE:
			s += _T("\tHyper-Threading Technology: not capable  \n\tMulti-core: Yes \n\tMulti-processor: ");
			if (PhysicalNum > 1)
				s += _T("yes\n");
			else
				s += _T("No\n");
		...
		case SINGLE_CORE_AND_HT_DISABLED:
			s += _T("\tHyper-Threading Technology: Disabled  \n\tMulti-core: No \n\tMulti-processor: ");
			if (PhysicalNum > 1)
				s += _T("yes\n");
			else
				s += _T("No\n");
		...
		case SINGLE_CORE_AND_HT_ENABLED:
			s += _T("\tHyper-Threading Technology: Enabled  \n\tMulti-core: No \n\tMulti-processor: ");
			if (PhysicalNum > 1)
				s += _T("yes\n");
			else
				s += _T("No\n");
	...
	*PhysicalNum  = 1;
	...
	while (dwAffinityMask && dwAffinityMask <= dwSystemAffinity)
	{
		if (SetThreadAffinityMask(GetCurrentThread(), dwAffinityMask))
		{
			...
			_stprintf(tmp, _T("AffinityMask = %d; Initial APIC = %d; Physical ID = %d, Core ID = %d,  SMT ID = %d\n"),
				    dwAffinityMask, apicID, tblPkgID[j], tblCoreID[j], tblSMTID[j]);
			_tcscat(g_s3Levels, tmp);
