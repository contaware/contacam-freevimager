    ...
    HDC dcSrc, int sx, int sy, int scx, int scy, int alpha)
{
	...
	// Create Src DIB section in shared memory
    BYTE* pSrcBits;
    HBITMAP hbmSrc;
	hbmSrc = ::CreateDIBSection (dcSrc, (BITMAPINFO*)&BMI,
        DIB_RGB_COLORS, (void**)&pSrcBits, 0, 0l);

	// Create Dst DIB section in shared memory
    BYTE * pDestBits;
    HBITMAP hbmDest;
    ...
    // Copy our destination and source bitmaps onto our DIBSections,
	...
	if (!::BitBlt(MemDC, 0, 0, cx, cy, dcDest, x, y, SRCCOPY))
	{
	...
	::SelectObject(MemDC, hbmSrc);
	if (!::StretchBlt(MemDC, 0, 0, cx, cy, dcSrc, sx, sy, scx, scy, SRCCOPY))
	{
		::SelectObject(MemDC, hOldBitmap);
		::DeleteObject(hbmSrc);    
		::DeleteObject(hbmDest);
	...
	// BitBlt From MemDC (with src Dib Section) To Dst DC
	BOOL res = ::BitBlt(dcDest, x, y, cx, cy, MemDC, 0, 0, SRCCOPY);
