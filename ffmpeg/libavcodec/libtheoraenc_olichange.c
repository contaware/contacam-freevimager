...
static av_cold int encode_init(AVCodecContext* avc_context)
{
    ...
    if (avc_context->flags & CODEC_FLAG_QSCALE) {
        // From 2..31 to 63..0
        t_info.quality = (av_clip(avc_context->global_quality / (float)FF_QP2LAMBDA, 2, 31) - 2.0f) * 2.1724f; // Oli changed
        t_info.quality = 63 - t_info.quality;
		t_info.target_bitrate = 0;
    } else {
        t_info.target_bitrate = av_clip(avc_context->bit_rate, 45000, 2000000); // Oli changed
        t_info.quality = 0;
    }
    ...
    void *data)
{
    ...
    switch (result) {
        ...
        case 1:
            ...
            if (theora_packet_iskeyframe(&o_packet) == 1)// Oli added this
				avc_context->coded_frame->key_frame = 1;// Oli added this
			else
				avc_context->coded_frame->key_frame = 0;// Oli added this
            break;
        default:
