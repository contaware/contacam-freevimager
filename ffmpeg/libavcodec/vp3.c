    ...
    for (y = ystart; y < yend; y++) {
        ...
        for (x = 0; x < width; x++) {
            ...
            if( s->all_fragments[fragment].coding_method != MODE_COPY )
            {
                ...
                if (x > 0) {
                    ff_vp3_h_loop_filter_c( // Oli changed
                        plane_data + 8*x,
                ...
                if (y > 0) {
                    ff_vp3_v_loop_filter_c( // Oli changed
                        plane_data + 8*x,
                        stride, bounding_values);
                }
...
static void render_slice(Vp3DecodeContext *s, int slice)
{
    ...
    for (plane = 0; plane < 3; plane++) {
        ...
        if (!s->flipped_image) stride = -stride;
        if (/*CONFIG_GRAY && */plane && (s->avctx->flags & CODEC_FLAG_GRAY)) // Oli commented
            continue;
                ...
                if (s->all_fragments[i].coding_method != MODE_COPY) {
                    ...
                    if (s->all_fragments[i].coding_method == MODE_INTRA) {
                        ...
                        s->dsp.idct_add(
                            ...
                            ff_vp3_idct_dc_add_c(output_plane + first_pixel, stride, block); // Oli changed
                        }
                    }
                } else {
