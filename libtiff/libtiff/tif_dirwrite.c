...
TIFFWritePrivateDataSubDirectory(	TIFF* tif,
									const TIFFFieldInfo field_info[],
									size_t field_info_count,
									int (*getFieldFn)(void*, TIFF *tif, ttag_t tag, ...),
									void* userdata)
{
	size_t i;
	uint16 dc;
	...
	maxdirsize = field_info_count * sizeof(TIFFDirEntry);
	data = (char*)_TIFFmalloc(maxdirsize);
	if (data == NULL)
	{
		TIFFErrorExt(tif->tif_clientdata, tif->tif_name,
	...
	/* Write data items */
	for (i = 0, dircount = 0, fip = (TIFFFieldInfo*)field_info, dir = (TIFFDirEntry*)data ;
		i < field_info_count ;
		i++, fip++)
	{
		if (TIFFWriteNormalSubTag(tif, dir, fip, getFieldFn, userdata))
		{
			dircount++;
			dir++;
		}
	...
	/* Check whether something has been written */
	if (dircount == 0)
		goto bad;

	/* Directory Size in Bytes */
	dirsize = dircount * sizeof(TIFFDirEntry);

	/* Now we've written all of the referenced data, and are about to
	   write the main directory structure. It's time to grab the tif_dataoff
	   value to remember where we wrote the directory */
	...
	if (tif->tif_flags & TIFF_SWAB)
	{
		 ...
		 * byte-swap indirect data.
		 */
		for (dc = dircount, dir = (TIFFDirEntry*)data ; dc ; dir++, dc--)
		{
			if (!((	dir->tdir_type == TIFF_ASCII		||
					dir->tdir_type == TIFF_UNDEFINED	||
					dir->tdir_type == TIFF_NOTYPE		||
	...
	TIFFSeekFile(tif, tif->tif_dataoff, SEEK_SET);
	if (!WriteOK(tif, &dircount, sizeof(dircount)))
	{
 ...
 */
static int
TIFFWriteNormalSubTag(	TIFF* tif,
						TIFFDirEntry* dir,
	...
	switch (fip->field_type)
	{
		...
		case TIFF_SSHORT:
			if (fip->field_passcount)
			{
				...
				if (wc == (uint16) TIFF_VARIABLE2)
				{
					if (!(*getFieldFn)(userdata, tif, fip->field_tag, &wc2, &wp))
						return 0;
					dir->tdir_count = wc2;
				}
				...
				{
					if (!(*getFieldFn)(userdata, tif, fip->field_tag, &wc, &wp))
						return 0;
					dir->tdir_count = wc;
				}
				if (!TIFFWriteShortArray(tif, dir, wp))
			...
			else
			{
				...
				{
					uint16 sv;
					if (!(*getFieldFn)(userdata, tif, fip->field_tag, &sv))
						return 0;
				...
				{
					uint16* wp;
					if (!(*getFieldFn)(userdata, tif, fip->field_tag, &wp))
						return 0;
					if (!TIFFWriteShortArray(tif, dir, wp))
						return 0;
				}
		...
		case TIFF_IFD:
			if (fip->field_passcount)
			{
				...
				if (wc == (uint16) TIFF_VARIABLE2)
				{
					if (!(*getFieldFn)(userdata, tif, fip->field_tag, &wc2, &lp))
						return 0;
					dir->tdir_count = wc2;
				}
				/* Assume TIFF_VARIABLE */
				...
				{
					if (!(*getFieldFn)(userdata, tif, fip->field_tag, &wc, &lp))
						return 0;
					dir->tdir_count = wc;
				}
				if (!TIFFWriteLongArray(tif, dir, lp))
			...
			else
			{
				...
				{
					/* XXX handle LONG->SHORT conversion */
					if (!(*getFieldFn)(userdata, tif, fip->field_tag, &dir->tdir_offset))
						return 0;
				}
				else
				{
					uint32* lp;
					if (!(*getFieldFn)(userdata, tif, fip->field_tag, &lp))
						return 0;
					if (!TIFFWriteLongArray(tif, dir, lp))
						return 0;
				}
		...
		case TIFF_SRATIONAL:
			if (fip->field_passcount)
			{
				...
				if (wc == (uint16) TIFF_VARIABLE2)
				{
					if (!(*getFieldFn)(userdata, tif, fip->field_tag, &wc2, &fp))
						return 0;
					dir->tdir_count = wc2;
				}
				/* Assume TIFF_VARIABLE */
				...
				{
					if (!(*getFieldFn)(userdata, tif, fip->field_tag, &wc, &fp))
						return 0;
					dir->tdir_count = wc;
				}
				if (!TIFFWriteRationalArray(tif, dir, fp))
			...
			else
			{
				...
				{
					float fv;
					if (!(*getFieldFn)(userdata, tif, fip->field_tag, &fv))
						return 0;
					if (!TIFFWriteRationalArray(tif, dir, &fv))
						return 0;
				}
				...
				{
					float* fp;
					if (!(*getFieldFn)(userdata, tif, fip->field_tag, &fp))
						return 0;
					if (!TIFFWriteRationalArray(tif, dir, fp))
						return 0;
				}
		...
		case TIFF_DOUBLE:
			if (fip->field_passcount)
			{
				...
				if (wc == (uint16) TIFF_VARIABLE2)
				{
					if (!(*getFieldFn)(userdata, tif, fip->field_tag, &wc2, &dp))
						return 0;
					dir->tdir_count = wc2;
				}
				/* Assume TIFF_VARIABLE */
				...
				{
					if (!(*getFieldFn)(userdata, tif, fip->field_tag, &wc, &dp))
						return 0;
					dir->tdir_count = wc;
				}
				if (!TIFFWriteDoubleArray(tif, dir, dp))
			...
			else
			{
				...
				{
					double dv;
					if (!(*getFieldFn)(userdata, tif, fip->field_tag, &dv))
						return 0;
					if (!TIFFWriteDoubleArray(tif, dir, &dv))
						return 0;
				}
				...
				{
					double* dp;
					if (!(*getFieldFn)(userdata, tif, fip->field_tag, &dp))
						return 0;
					if (!TIFFWriteDoubleArray(tif, dir, dp))
						return 0;
				}
		...
		case TIFF_ASCII:
			...
			if (fip->field_passcount)
			{
				if (!(*getFieldFn)(userdata, tif, fip->field_tag, &wc, &cp))
					return 0;
			}
			else
			{
				if (!(*getFieldFn)(userdata, tif, fip->field_tag, &cp))
					return 0;
			}
			dir->tdir_count = (uint32) (strlen(cp) + 1);
			if (!TIFFWriteByteArray(tif, dir, cp))
		...
		case TIFF_SBYTE:          
			if (fip->field_passcount)
			{
				...
				if (wc == (uint16) TIFF_VARIABLE2)
				{
					if (!(*getFieldFn)(userdata, tif, fip->field_tag, &wc2, &cp))
						return 0;
					dir->tdir_count = wc2;
				}
				/* Assume TIFF_VARIABLE */
			...
			else
			{
				...
				{
					char cv;
					if (!(*getFieldFn)(userdata, tif, fip->field_tag, &cv))
						return 0;
					if (!TIFFWriteByteArray(tif, dir, &cv))
						return 0;
				}
