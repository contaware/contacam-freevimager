...
TIFFRGBAImageBegin(TIFFRGBAImage* img, TIFF* tif, int stop, char emsg[1024])
{
	...
	// Oli Added
	img->hProgressWnd = NULL;
	img->bProgressSend = 0;
	img->hKillEvent = NULL;
	img->bForceVerticalFlip = 0;
...
gtTileContig(TIFFRGBAImage* img, uint32* raster, uint32 w, uint32 h)
{
    ...
    for (row = 0; row < h; row += nrow)
    {
		...
		{
			if (img->hKillEvent &&
				WaitForSingleObject(img->hKillEvent, 0) == WAIT_OBJECT_0)
			{
				DIB_END_PROGRESS((HWND)img->hProgressWnd);
				return (0);
			}
			DIB_PROGRESS((HWND)img->hProgressWnd, img->bProgressSend, row, h);
		}
