...
void CPeakMeterCtrl::ResetControl()
{
	...
	for (int i = 0 ; i < (int)m_MeterData.size() ; i++)
	{
		m_MeterData.at(i) = pm;
	}
}

COLORREF CPeakMeterCtrl::LightenColor(const COLORREF crColor, BYTE byIncreaseVal)
{
	WORD wRed = GetRValue(crColor);
	WORD wGreen = GetGValue(crColor);
	WORD wBlue = GetBValue(crColor);

	wRed = (wRed + byIncreaseVal) <= 255 ? BYTE(wRed + byIncreaseVal) : 255;
	wGreen = (wGreen + byIncreaseVal)	<= 255 ? BYTE(wGreen + byIncreaseVal) : 255;
	wBlue = (wBlue + byIncreaseVal) <= 255 ? BYTE(wBlue + byIncreaseVal) : 255;

	return RGB(wRed, wGreen, wBlue);
}

COLORREF CPeakMeterCtrl::DarkenColor(const COLORREF crColor, BYTE byReduceVal)
{
...
bool CPeakMeterCtrl::Start(UINT uDelay)
{
	...
	{
		MMRESULT mmresult;
		mmresult = ::timeSetEvent(uDelay, PEAKMETER_TIMER_RESOLUTION, PeakTimerProc, (DWORD)this, TIME_PERIODIC);
...
{
	CPeakMeterCtrl* pThis = (CPeakMeterCtrl*)dwUser;
	ASSERT(pThis != NULL);
