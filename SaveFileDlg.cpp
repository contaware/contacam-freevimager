...
void CSaveFileDlg::OnInitDone()
{
	if (m_bShowJPEGCompression)
	{
		...
		GetParent()->SetWindowPos(	NULL, 0, 0, rcDialog.Width(),
									rcDialog.Height() + ::SystemDPIScale(SAVEFILEDLG_SLIDER_HEIGHT + SAVEFILEDLG_BOTTOM_OFFSET),
									SWP_NOMOVE | SWP_NOZORDER);
		ScreenToClient(rcDialog);

		// Slider Label
		CRect rcSliderLabel(::SystemDPIScale(SAVEFILEDLG_LEFT_OFFSET),
							rcDialog.bottom,
		...
		CRect rcSlider(		rcSliderLabel.right,
							rcSliderLabel.top,
							rcDialog.Width() - ::SystemDPIScale(SAVEFILEDLG_SLIDERTEXT_WIDTH + SAVEFILEDLG_RIGHT_OFFSET),
		...
		CRect rcSliderText(	rcSlider.right,
							rcSlider.top,
							rcSlider.right + ::SystemDPIScale(SAVEFILEDLG_SLIDERTEXT_WIDTH),
							rcSlider.bottom);
		CString sSliderText;
