...
__forceinline unsigned int ReverseBits(unsigned int p_nIndex, unsigned int p_nBits);
...
#define INTENSITY(re,im)			(sqrt(double((re)*(re))+((im)*(im))))
#define INTENSITY_SQRD(re,im)		((re)*(re)+(im)*(im))
#define INTENSITY_DECIBELS(re,im)	(20.0 * log10(double(INTENSITY_SQRD(re,im))))
