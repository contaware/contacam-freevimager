...
CString cdxCDynamicWndEx::MakeFullProfile(LPCTSTR lpszBase, const CString & str)
{
	CString	s		=	lpszBase;
...
void cdxCDynamicWndEx::OnInitialized()
{
	...
	{
		if(!RestoreWindowPosition(MakeFullProfile(M_lpszAutoPosProfileSection,m_strAutoPos),rflg_all))
		{
			Window()->CenterWindow();
			StretchWindow(10);
		}
...
void cdxCDynamicWndEx::OnDestroying()
{
	if(!m_strAutoPos.IsEmpty() && IsWindow())
		StoreWindowPosition(MakeFullProfile(M_lpszAutoPosProfileSection,m_strAutoPos));
}
