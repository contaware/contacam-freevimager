...
int CH263Enc::Dct(int* block, int* coeff)
{
  int        j1, i, j, k;
  float	b[8];
  float 	b1[8];
  float 	d[8][8];
  float 	f0=.7071068f,f1=.4903926f,f2=.4619398f,f3=.4157348f,f4=.3535534f;
  float 	f5=.2777851f,f6=.1913417f,f7=.0975452f;
 ...
 * --------------------------------------------------------------
 */
void CH263Enc::mp_fwd_dct_fast(Block data2d, Block dest2d)
{
    ...
    for (rowctr = DCTSIZE - 1; rowctr >= 0; rowctr--) {
      ...
      outptr[ m_zigzag[0][columncounter] ] = (int32) UNFIXH((tmp10 + tmp11) * SIN_1_4);
      outptr[ m_zigzag[4][columncounter] ] = (int32) UNFIXH((tmp10 - tmp11) * COS_1_4);
      
      outptr[ m_zigzag[2][columncounter] ] = (int32) UNFIXH(tmp13 * COS_1_8 + tmp12 * SIN_1_8);
      outptr[ m_zigzag[6][columncounter] ] = (int32) UNFIXH(tmp13 * SIN_1_8 - tmp12 * COS_1_8);
      ...
      outptr[ m_zigzag[1][columncounter] ] = (int32) UNFIXH(tmp17 * OCOS_1_16 + tmp14 * OSIN_1_16);
      outptr[ m_zigzag[7][columncounter] ] = (int32) UNFIXH(tmp17 * OCOS_7_16 - tmp14 * OSIN_7_16);
      outptr[ m_zigzag[5][columncounter] ] = (int32) UNFIXH(tmp26 * OCOS_5_16 + tmp25 * OSIN_5_16);
      outptr[ m_zigzag[3][columncounter] ] = (int32) UNFIXH(tmp26 * OCOS_3_16 - tmp25 * OSIN_3_16);
...
int CH263Enc::idct(int* coeff, int* block)
{
  int 	    j1, i, j;
...
void CH263Enc::idctcol(int *blk)
{
  ...
  /* fourth stage */
  blk[0] = m_iclp[(x7+x1)>>14];
  blk[8] = m_iclp[(x3+x2)>>14];
  blk[16] = m_iclp[(x0+x4)>>14];
  blk[24] = m_iclp[(x8+x6)>>14];
  blk[32] = m_iclp[(x8-x6)>>14];
  blk[40] = m_iclp[(x0-x4)>>14];
  blk[48] = m_iclp[(x3-x2)>>14];
  blk[56] = m_iclp[(x7-x1)>>14];
}
...
{
  int i;
  int *block_ptr, *m_zigzag_ptr;

  block_ptr = block; m_zigzag_ptr = &(m_zigzag[0][0]);

  for (i = 0; i < 8; i++) {
    *(block_ptr++) = *(coeff + *(m_zigzag_ptr++));
    *(block_ptr++) = *(coeff + *(m_zigzag_ptr++));
    *(block_ptr++) = *(coeff + *(m_zigzag_ptr++));
    *(block_ptr++) = *(coeff + *(m_zigzag_ptr++));
    *(block_ptr++) = *(coeff + *(m_zigzag_ptr++));
    ...
    *(block_ptr++) = *(coeff + *(m_zigzag_ptr++));
    *(block_ptr++) = *(coeff + *(m_zigzag_ptr++));
    /* WAS:     block[j + i*8] = (int) *( coeff + m_zigzag[i][j]); */
  }
...
void CH263Enc::init_idct()
{
  ...
  for (freq=0; freq < 8; freq++)
  {
    ...
    for (time=0; time<8; time++)
      m_cos[freq][time] = scale*cos((PI/8.0)*freq*(time + 0.5));
  }
}

/* perform IDCT matrix multiply for 8x8 coefficient block */

void CH263Enc::idct(int *coeff, int *block)
{
  int i, j, k, v;
  ...
  for (i=0; i<8; i++)
    for (j=0; j<8; j++)
    {
      ...
      for (k=0; k<8; k++)
        partial_product+= m_cos[k][j]*tmp2[8*i+k];
  ...
  for (j=0; j<8; j++)
    for (i=0; i<8; i++)
    {
      ...
      for (k=0; k<8; k++)
        partial_product+= m_cos[k][i]*tmp[8*k+j];
