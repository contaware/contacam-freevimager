...
int CH263Enc::SAD_HalfPixelMacroblock(unsigned int *ii,
			    unsigned int *curr,
...
void CH263Enc::FullMotionEstimation(unsigned int *curr, unsigned int *prev_ipol, 
				    ...
				    pixel of this MB in whole frame curr */
{
  ...
  curr_image_MB = curr + m_pels*y_curr + x_curr;
  ...
  if(m_search_p_frames == CPARAM_EXHAUSTIVE) {
    /* This is the spriral search variant of exhaustive seach */
    /* First determine zero MV value SAD */
    xblock = CenterX; yblock = CenterY;

    sad = SAD_HalfPixelMacroblock(prev_ipol + xblock + yblock*m_pels*2,
				  curr_image_MB,
				  m_pels, Min_FRAME) - PREF_NULL_VEC;
  ...
  } else { /* m_search_p_frames == DEF_LOGARITHMIC_SEARCH */
    ...
    sad = SAD_HalfPixelMacroblock(prev_ipol + CenterX + CenterY*m_pels*2,
				  curr_image_MB,m_pels, 
	...
	for(yblock = CenterY - StepSizeY; yblock <= CenterY + StepSizeY &&
	  ...
	  sad = SAD_HalfPixelMacroblock(prev_ipol + xblock + yblock*m_pels*2,
					curr_image_MB,m_pels, Min_FRAME); 
	  if (sad < Min_FRAME) {
...
int CH263Enc::SAD_HalfPixelMacroblock2(unsigned int *ii,
			    unsigned int *curr,
			    int pixels_on_line, int Min_SAD)
{
  int j = 16;
...
unsigned int* CH263Enc::LoadArea(unsigned int *im, int x, int y, 
...
void CH263Enc::FindMB(int x, int y, unsigned int *image, unsigned int MB[16][16])
