...
void CH263Enc::CountBitsMB(int Mode, int COD, int CBP, int CBPB, Pict *pic, Bits *bits)
{
  int cbpy, cbpcm, length;

  /* COD */
  if (m_trace) {
    fprintf(m_tf,"MB-nr: %d",pic->MB);
  ...
  if (m_trace) {
    fprintf(m_tf,"CBPCM (CBP=%d) (cbpcm=%d): ",CBP,cbpcm);
  }
  if (pic->picture_coding_type == PCT_INTRA) {
    length = Encode(cbpcm, m_vlc_cbpcm_intra);
  } else {
    length = Encode(cbpcm, m_vlc_cbpcm);
  }
  bits->CBPCM += length;
  ...
  if (Mode == MODE_INTRA || Mode == MODE_INTRA_Q) /* Intra */
    cbpy = cbpy^15;
  if (m_trace) {
    fprintf(m_tf,"CBPY (CBP=%d) (cbpy=%d): ",CBP,cbpy);
  }
  length = Encode(cbpy, m_vlc_cbpy);
  bits->CBPY += length;
}
...
int CH263Enc::CountBitsSlice(int slice, int quant)
{
  int bits = 0;
...
void CH263Enc::CountBitsCoeff(int *qcoeff, int Mode, int CBP, Bits *bits, int ncoeffs)
...
int CH263Enc::CodeCoeff(int Mode, int *qcoeff, int block, int ncoeffs)
{
  int j, bits;
  ...
  for (j = block*ncoeffs; j< (block + 1)*ncoeffs; j++) {
	...
	if (!first) {
	  ...
	  if (prev_level  < 13 && prev_run < 64) {
	    length = Encode(prev_ind, m_vlc_3d);

	  } else
	    length = 0;
	  if (length == 0) {  /* Escape coding */
	    if (m_trace) {
	      fprintf(m_tf,"Escape coding:\n");
	    }
	    if (prev_s == 1) {prev_level = (prev_level^0xff)+1;}
	    Encode(ESCAPE, m_vlc_3d);
	    mputv(1,0);
	    mputv(6,prev_run);
	    mputv(8,prev_level);
	    bits += 22;
...
int CH263Enc::FindCBP(int *qcoeff, int Mode, int ncoeffs)
{
...
void CH263Enc::CountBitsVectors(MotionVector *MV_ptr, Bits *bits, 
		      int x, int y, int Mode, int newgob, Pict *pic)
{
  ...
  for (block = start; block <= stop;  block++) {
    ...
    if (m_trace) {
      fprintf(m_tf,"Vectors:\n");
    }
    bits->vec += Encode(x_vec, m_vlc_mv);
    bits->vec += Encode(y_vec, m_vlc_mv);

    if (m_trace) {
      if (x_vec > 31) x_vec -= 64;
      if (y_vec > 31) y_vec -= 64;
      fprintf(m_tf,"(x,y) = (%d,%d) - ",
	      (2*(*MV_xy).x + (*MV_xy).x_half),
	      (2*(*MV_xy).y + (*MV_xy).y_half));
      fprintf(m_tf,"(Px,Py) = (%d,%d)\n", pmv0,pmv1);
      fprintf(m_tf,"(x_diff,y_diff) = (%d,%d)\n",x_vec,y_vec);
    }
  }
}

void CH263Enc::FindPMV(MotionVector *MV_ptr, int x, int y, 
	     int *pmv0, int *pmv1, int block, int newgob, int half_pel)
{
  ...
  if (half_pel) {
    ...
    p1 = (x > 0) ? 
      2*((*(MV_ptr + yin1*m_mbc + xin1)).x) + (*(MV_ptr+ yin1*m_mbc + xin1)).x_half : 0;
    p2 = (y > 0) ? 
      2*((*(MV_ptr + yin2*m_mbc + xin2)).x) + (*(MV_ptr + yin2*m_mbc + xin2)).x_half : 2*NO_VEC;
    if((y > 0) && (x < m_mbc - 1))
      p3 = 2*((*(MV_ptr+yin3*m_mbc + xin3)).x) + ((*(MV_ptr + yin3*m_mbc + xin3)).x_half);
    else if(x == m_mbc - 1)
      p3 = 0;
    else			/* y == 0 && x != m_mbc - 1 */
      p3 = 2*NO_VEC;
    ...
    p1 =  (x > 0) ? 
      (2*((*(MV_ptr + yin1*m_mbc + xin1)).x)) : 0;
    p2 =  (y > 0) ? 
      (2*((*(MV_ptr + yin2*m_mbc + xin2)).x)) : 2*NO_VEC;
    if((y > 0) && (x < m_mbc - 1))
      p3 = 2*((*(MV_ptr + yin3*m_mbc + xin3)).x);
    else if(x == m_mbc - 1)
      p3 = 0;
    else			/* y == 0 && x != m_mbc - 1 */
      p3 = 2*NO_VEC;
  ...
  *pmv0 = p1+p2+p3 - mmax(p1,mmax(p2,p3)) - mmin(p1,mmin(p2,p3));
  if (half_pel) {
    p1 =  (x > 0) ? 
      (2*((*(MV_ptr + yin1*m_mbc + xin1)).y)) + ((*(MV_ptr + yin1*m_mbc + xin1)).y_half) : 0;
    p2 =  (y > 0) ? 
      2*((*(MV_ptr + yin2*m_mbc + xin2)).y) + ((*(MV_ptr + yin2*m_mbc + xin2)).y_half) : 2*NO_VEC;

    if((y > 0) && (x < m_mbc - 1))
      p3 = 2*((*(MV_ptr + yin3*m_mbc + xin3)).y) + ((*(MV_ptr + m_mbc*yin3 + xin3)).y_half);
    else if(x == m_mbc - 1)
      p3 = 0;
    else			/* y == 0 && x != m_mbc - 1 */
      p3 = 2*NO_VEC;
    ...
    p1 = (x > 0) ?
      (2*((*(MV_ptr + yin1*m_mbc + xin1)).y)) : 0;
    p2 =  (y > 0) ?
      (2*((*(MV_ptr + yin2*m_mbc + xin2)).y)) : 2*NO_VEC;
    if((y > 0) && (x < m_mbc - 1))
      p3 = 2*((*(MV_ptr + yin3*m_mbc + xin3)).y);
    else if(x == m_mbc - 1)
      p3 = 0;
    else			/* y == 0 && x != m_mbc - 1 */
      p3 = 2*NO_VEC;
...
void CH263Enc::ZeroBits(Bits *bits)
{
...
void CH263Enc::ZeroRes(Results *res)
{
#ifndef MINIMAL_REPORT
  res->SNR_l = 0;
  res->SNR_Cr = 0;
...
void CH263Enc::AddBits(Bits *total, Bits *bits)
{
#ifndef MINIMAL_REPORT
  total->Y += bits->Y;
  total->C += bits->C;
...
void CH263Enc::AddRes(Results *total, Results *res, Pict *pic)
{
#ifndef MINIMAL_REPORT
...
void CH263Enc::AddBitsPicture(Bits *bits)
{
#ifndef MINIMAL_REPORT
  bits->total = 
    bits->Y + 
...
void CH263Enc::ZeroVec(MotionVector *MV)
{
  MV->x = 0;
  MV->y = 0;
  MV->x_half = 0;
...
void CH263Enc::MarkVec(MotionVector *MV)
{
  MV->x = NO_VEC;
  MV->y = NO_VEC;
  MV->x_half = 0;
...
void CH263Enc::CopyVec(MotionVector *MV2, MotionVector *MV1)
{
  MV2->x = MV1->x;
...
int CH263Enc::EqualVec(MotionVector *MV2, MotionVector *MV1)
{
  if (MV1->x != MV2->x)
    return 0;
  if (MV1->y != MV2->y)
...
int CH263Enc::CountBitsPicture(Pict *pic)
{
  int bits = 0;

  /* Picture start code */
  if (m_trace) {
    fprintf(m_tf,"picture_start_code: ");
  }
  ...
  /* Group number */
  if (m_trace) {
    fprintf(m_tf,"Group number in picture header: ");
  }
  mputv(5,0); 
  bits += 5;
 ...
 /* bit 1 */
  ...
  /* bit 2 */
  if (m_trace) {
    fprintf(m_tf,"always zero for distinction with H.261\n");
  }
  mputv(1,0);
  bits += 1;
  ...
  /* bit 3 */
  if (m_trace) {
    fprintf(m_tf,"split_screen_indicator: ");
  }
  ...
  /* bit 4 */
  if (m_trace) {
    fprintf(m_tf,"document_camera_indicator: ");
  }
  mputv(1,0);
  bits += 1;
  ...
  /* bit 5 */
  if (m_trace) {
    fprintf(m_tf,"freeze_picture_release: ");
  }
  mputv(1,0);
  bits += 1;
  ...
  /* bit 9 */
  if (m_trace) {
    fprintf(m_tf,"picture_coding_type: ");
  }
  ...
  /* bit 10 */
  if (m_trace) {
    fprintf(m_tf,"m_mv_outside_frame: ");
  }
  ...
  /* bit 12 */
  if (m_trace) {
    fprintf(m_tf,"adv_pred_mode: ");
  }
  mputv(1,m_advanced); /* Advanced Prediction mode */
  bits += 1;

  /* bit 13 */
  if (m_trace) {
    fprintf(m_tf,"PB-coded: "); /* PB-frames mode */
  }
  mputv(1,pic->PB);
  bits += 1;
