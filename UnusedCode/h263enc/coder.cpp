...
void CH263Enc::Clip(MB_Structure *data) 
{
  int n;
  int *mb_ptr = (int *) data;
...
void CH263Enc::CodeIntraH263(CParam *params, Bits *bits)
{
  ...
  m_pic->QUANT = params->Q_intra;
  m_pic->picture_coding_type = PCT_INTRA;

  bits->header += CountBitsPicture(m_pic);
  ...
  for ( j = 0; j < m_mbr; j++) {
    ...
    for ( i = 0; i < m_mbc; i++) {
      ...
      CountBitsMB(Mode,0,CBP,0,m_pic,bits);
  ...
  m_pic->QP_mean = (float)(params->Q_intra);
...
int* CH263Enc::MB_EncodeAndFindCBP(MB_Structure *mb_orig, int QP, int I, int *CBP)
{				
  int		i, j, k, l, row, col;
  int		fblock[64];
  int		coeff[384];
...
int CH263Enc::MB_Decode(int *qcoeff, MB_Structure *mb_recon, int QP, int I)
{
  int	i, j, k, l, row, col;
  int	*iblock;
  int	*qcoeff_ind;
...
void CH263Enc::FillLumBlock( int x, int y, unsigned int *image, MB_Structure *data)
{
  int n, m;
				/* OPTIMIZE HERE */
...
void CH263Enc::FillChromBlock(int x_curr, int y_curr, unsigned int *image,
		    MB_Structure *data)
{
  int n;
  register int m;
  ...
  for (n = 0; n < (MB_SIZE>>1); n++)
    for (m = 0; m < (MB_SIZE>>1); m++) {
      data->Cr[n][m] = 
	(int) (*(image + m_vskip + x+m + (y+n)*m_cpels));
      data->Cb[n][m] = 
	(int) (*(image + m_uskip + x+m + (y+n)*m_cpels));
    }
}
...
void CH263Enc::ZeroMBlock(MB_Structure *data)
{
  int n;
  register int m;
...
void CH263Enc::ReconImage(int i, int j, MB_Structure *data, unsigned int *recon)
{
  int n;
  int x_curr, y_curr;
  ...
  recon_ptr = recon + x_curr + y_curr*m_pels;
  ...
  for (n = 0; n < MB_SIZE; n++) {
    ...
    * (long *) recon_ptr = * (long *) lum_ptr;
    recon_ptr += 2; lum_ptr += 2;
    * (long *) recon_ptr = * (long *) lum_ptr;
    recon_ptr += m_pels - 14; lum_ptr += 2;
    /* Was: for every m = 0..15 :
     *(recon->lum + x_curr+m + (y_curr+n)*m_pels) = *(lum_ptr++);
     */
#else
    ...
    * (int *) recon_ptr = * (int *) lum_ptr;
    ...
    recon_ptr += m_pels - 15; lum_ptr++;
#endif
  }

  recon_Cb_ptr = recon+m_uskip+ (x_curr>>1) + (y_curr>>1)*m_cpels;
  recon_Cr_ptr = recon+m_vskip+ (x_curr>>1) + (y_curr>>1)*m_cpels;
  Cb_ptr = &(data->Cb[0][0]);
  Cr_ptr = &(data->Cr[0][0]);
  ...
  for (n = 0; n < MB_SIZE>>1; n++) {
    ...
    * (long *) recon_Cr_ptr = * (long *) Cr_ptr;
    recon_Cr_ptr += 2; Cr_ptr += 2;
    * (long *) recon_Cb_ptr = * (long *) Cb_ptr;
    recon_Cb_ptr += m_cpels - 6; Cb_ptr += 2;
    * (long *) recon_Cr_ptr = * (long *) Cr_ptr;
    recon_Cr_ptr += m_cpels - 6; Cr_ptr += 2;
#else
    ...
    * (int *) recon_Cr_ptr = * (int *) Cr_ptr;
    ...
    recon_Cb_ptr += m_cpels - 7; Cb_ptr ++;
    recon_Cr_ptr += m_cpels - 7; Cr_ptr ++;
#endif
  }
  /* WAS:   
     for (m = 0; m < MB_SIZE>>1; m++) {
     *(recon->Cr + (x_curr>>1)+m + ((y_curr>>1)+n)*m_cpels) = data->Cr[n][m];
     *(recon->Cb + (x_curr>>1)+m + ((y_curr>>1)+n)*m_cpels) = data->Cb[n][m];
     }
     */
}
...
void CH263Enc::ReconCopyImage (int i, int j, unsigned int *recon, unsigned int *prev_recon)
{
  int n;
  int x_curr, y_curr;
  ...
  lum_now = recon + x_curr + y_curr*m_pels;
  lum_prev = prev_recon + x_curr + y_curr*m_pels;
  ...
  cb_now = recon+m_uskip + (x_curr>>1) + (y_curr>>1)*m_cpels;
  cr_now = recon+m_vskip + (x_curr>>1) + (y_curr>>1)*m_cpels;
  cb_prev = prev_recon+m_uskip+ (x_curr>>1) + (y_curr>>1)*m_cpels;
  cr_prev = prev_recon+m_vskip + (x_curr>>1) + (y_curr>>1)*m_cpels;
...
void CH263Enc::InterpolateImage(unsigned int *image, unsigned int *ipol_image, 
...
void CH263Enc::FullMotionEstimatePicture(unsigned int *curr, unsigned int *prev, 
			       ...
			       int *EncodeThisBlock)
{
  ...
  for(j = 0; j < m_mbr; j++) {
   ...
   for(i = 0; i < m_mbc; i++) {
     current_MV = MV_ptr + j*m_mbc + i;

     if(advanced_method && !*(EncodeThisBlock + j*m_mbc + i) ) {
...
void CH263Enc::CodeInterH263(CParam *params, Bits *bits)
{
  ...
  m_search_p_frames = params->search_method;

  MV = (MotionVector*) malloc(sizeof(MotionVector)*m_mbc*m_mbr);

  new_recon = (unsigned int*) malloc(m_sizeof_frame);
  ...
  InterpolateImage(prev_recon,params->interpolated_lum,m_pels,m_lines);
  ...
  for ( j = 0; j < m_mbr; j++) {
    ...
    if (j == 0) {
      m_pic->QUANT = params->Q_inter;
      m_pic->picture_coding_type = PCT_INTER;
      bits->header += CountBitsPicture(m_pic);
    }
    else if (m_pic->use_gobsync && j%m_pic->use_gobsync == 0) {
	...
	if(Mode == MODE_SKIP) {
	  ...
	  MV[j*m_mbc + i].Mode = Mode;

	  ZeroVec(&(MV[j*m_mbc + i]));
 
	  CBP = CBPB = 0;
 
	  /* Now send code for "skip this MB" */
	  CountBitsMB(Mode,1,CBP,CBPB,m_pic,bits);
	  ...
	  if((CBP==0) && (EqualVec(&(MV[j*m_mbc+i]),&ZERO)) &&
	     (Mode == MODE_INTER) ) {
	    ...
	    /* Normal MB */
	    /* VLC */
	    CountBitsMB(Mode,0,CBP,CBPB,m_pic,bits);

	    if (Mode == MODE_INTER) {
	      bits->no_inter++;
	      CountBitsVectors(MV, bits, i, j, Mode, newgob, m_pic);
	    } else {
	      /* MODE_INTRA */
	      bits->no_intra++;
	    }
  ...
  m_pic->QP_mean = (float)(params->Q_inter);
