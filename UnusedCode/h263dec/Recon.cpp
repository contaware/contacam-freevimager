...
void CH263Dec::reconstruct(int bx,int by,int P,int bdx,int bdy)
{
  ...
  x = bx/16+1; y = by/16+1;
  lx = m_coded_picture_width;

  if (m_mv_outside_frame) {
    lx2 = m_coded_picture_width + 64;
    src[0] = m_edgeframe[0];
    src[1] = m_edgeframe[1];
    src[2] = m_edgeframe[2];
  }
  else {
    lx2 = m_coded_picture_width;
    src[0] = m_oldrefframe[0];
    src[1] = m_oldrefframe[1];
    src[2] = m_oldrefframe[2];
  }
  ...
  if (P) {
    ...
    if (m_adv_pred_mode) {
      ...
      for (comp = 0; comp < 4; comp++) {
        xp = bx + ((comp&1)<<3);
        yp = by + ((comp&2)<<2);
        recon_comp_obmc(src[0],m_newframe[0], lx,lx2,comp,w,h,xp,yp);
      }
      if (mode == MODE_INTER4V) {

        sum = m_MV[0][1][y][x]+m_MV[0][2][y][x]+ m_MV[0][3][y][x]+m_MV[0][4][y][x];
        dx = sign(sum)*(m_roundtab[abs(sum)%16] + (abs(sum)/16)*2);

        sum = m_MV[1][1][y][x]+m_MV[1][2][y][x]+ m_MV[1][3][y][x]+m_MV[1][4][y][x];
        dy = sign(sum)*(m_roundtab[abs(sum)%16] + (abs(sum)/16)*2);
      ...
      by>>=1;
      /* Chroma */
      recon_comp(src[1],m_newframe[1], lx,lx2,w,h,bx,by,dx,dy,1);
      recon_comp(src[2],m_newframe[2], lx,lx2,w,h,bx,by,dx,dy,2);
    }
      ...
      /* Y */
      recon_comp(src[0],m_newframe[0], lx,lx2,w,h,bx,by,dx,dy,0);
    ...
    if (m_adv_pred_mode) {
      if (mode == MODE_INTER4V) {
        ...
        xvec = yvec = 0;
        for (comp = 0; comp < 4; comp++) {
          xvec += dx = (m_trb)*m_MV[0][comp+1][y][x]/m_trd + bdx;
          yvec += dy = (m_trb)*m_MV[1][comp+1][y][x]/m_trd + bdy;
          xp = bx + ((comp&1)<<3);
          yp = by + ((comp&2)<<2);
          recon_comp(src[0],m_bframe[0], lx,lx2,w,h,xp,yp,dx,dy,0);
        }

        /* chroma rounding (table 16/H.263) */
        dx = sign(xvec)*(m_roundtab[abs(xvec)%16] + (abs(xvec)/16)*2);
        dy = sign(yvec)*(m_roundtab[abs(yvec)%16] + (abs(yvec)/16)*2);

        lx>>=1;bx>>=1; lx2>>=1;
        by>>=1;
        /* Chroma */
        recon_comp(src[1],m_bframe[1], lx,lx2,w,h,bx,by,dx,dy,1);
        recon_comp(src[2],m_bframe[2], lx,lx2,w,h,bx,by,dx,dy,2);
      }
      else {  /* adv_pred_mode but 16x16 vector */
        w = 16; h = 16;

        dx = (m_trb)*m_MV[0][0][y][x]/m_trd + bdx;
        dy = (m_trb)*m_MV[1][0][y][x]/m_trd + bdy;
        /* Y */
        recon_comp(src[0],m_bframe[0], lx,lx2,w,h,bx,by,dx,dy,0);
...
void CH263Dec::recon_comp(unsigned char* src,unsigned char* dst,int lx,int lx2,int w,int h,int x,int y,int dx,int dy,int chroma)
{
  int xint, xh, yint, yh;
  unsigned char *s, *d;
...
void CH263Dec::rec(unsigned char* s,unsigned char* d,int lx,int lx2,int h)
{
  int j;
...
void CH263Dec::recc(unsigned char* s,unsigned char* d,int lx,int lx2,int h)
{
  int j;
...
void CH263Dec::reco(unsigned char* s,int *d,int lx,int lx2,int addflag,int c,int xa,int xb,int ya,int yb)
{
  int i,j;
  const int *om;
...
void CH263Dec::rech(unsigned char* s,unsigned char* d,int lx,int lx2,int h)
{
  unsigned char *dp,*sp;
  int j;
  unsigned int s1,s2;
...
void CH263Dec::rechc(unsigned char* s,unsigned char* d,int lx,int lx2,int h)
{
  unsigned char *dp,*sp;
  int j;
  unsigned int s1,s2;
...
void CH263Dec::recho(unsigned char* s,int *d,int lx,int lx2,int addflag,int c,int xa,int xb,int ya,int yb)
{
  int *dp;
  const int *om;
  unsigned char *sp;
  ...
  if (!addflag) {
    ...
    for (j = ya; j < yb; j++) {
      for (i = xa; i < xb; i++) {
        dp[i] += (((unsigned int)(sp[i] + sp[i+1]+1))>>1)*m_OM[c][j][i];
      }
      sp+= lx2;
      dp+= lx;
      om+= 8;
...
void CH263Dec::recv(unsigned char* s,unsigned char* d,int lx,int lx2,int h)
{
...
void CH263Dec::recvc(unsigned char* s,unsigned char* d,int lx,int lx2,int h)
{
  unsigned char *dp,*sp,*sp2;
  int j;
...
void CH263Dec::recvo(unsigned char* s,int *d,int lx,int lx2,int addflag,int c,int xa,int xb,int ya,int yb)
{
  int *dp;
  const int *om;
...
void CH263Dec::rec4(unsigned char* s,unsigned char* d,int lx,int lx2,int h)
{
  unsigned char *dp,*sp,*sp2;
  int j;
...
void CH263Dec::rec4c(unsigned char *s ,unsigned char *d,int lx,int lx2,int h)
{
  unsigned char *dp,*sp,*sp2;
  int j;
  unsigned int s1,s2,s3,s4;
...
void CH263Dec::rec4o(unsigned char* s,int *d,int lx,int lx2,int addflag,int c,int xa,int xb,int ya,int yb)
{
  int *dp;
  const int *om;
  unsigned char *sp,*sp2;
...
void CH263Dec::recon_comp_obmc(unsigned char* src,unsigned char* dst,int lx,int lx2,int comp,int w,int h,int x,int y)
{
  int j,k;
  int xmb,ymb;
  int c8,t8,l8,r8;
  ...
  c8  = (m_modemap[ymb][xmb] == MODE_INTER4V ? 1 : 0);

  t8  = (m_modemap[ymb-1][xmb] == MODE_INTER4V ? 1 : 0);
  ti8 = (m_modemap[ymb-1][xmb] == MODE_INTRA ? 1 : 0);
  ti8 = (m_modemap[ymb-1][xmb] == MODE_INTRA_Q ? 1 : ti8);

  l8  = (m_modemap[ymb][xmb-1] == MODE_INTER4V ? 1 : 0);
  li8 = (m_modemap[ymb][xmb-1] == MODE_INTRA ? 1 : 0);
  li8 = (m_modemap[ymb][xmb-1] == MODE_INTRA_Q ? 1 : li8);
  
  r8  = (m_modemap[ymb][xmb+1] == MODE_INTER4V ? 1 : 0);
  ri8 = (m_modemap[ymb][xmb+1] == MODE_INTRA ? 1 : 0);
  ri8 = (m_modemap[ymb][xmb+1] == MODE_INTRA_Q ? 1 : ri8);
  ...
  nx[0] = m_MV[0][c8 ? comp + 1 : 0][ymb][xmb];
  ny[0] = m_MV[1][c8 ? comp + 1 : 0][ymb][xmb];
  
  nx[1] = m_MV[0][vect][yit][xit]; ny[1] = m_MV[1][vect][yit][xit];
  nx[2] = m_MV[0][vecb][yib][xib]; ny[2] = m_MV[1][vecb][yib][xib];
  nx[3] = m_MV[0][vecr][yir][xir]; ny[3] = m_MV[1][vecr][yir][xir];
  nx[4] = m_MV[0][vecl][yil][xil]; ny[4] = m_MV[1][vecl][yil][xil];
