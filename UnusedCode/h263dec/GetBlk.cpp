...
void CH263Dec::getblock(int comp,int mode)
{
  int val, i, j, sign;
  unsigned int code;
  const VLCtabI *tab;
  ...
  for (i=(mode==0); ; i++) {
    code = showbits(12);
    if (code>=512)
      tab = &m_DCT3Dtab0[(code>>5)-16];
    else if (code>=128)
      tab = &m_DCT3Dtab1[(code>>2)-32];
    else if (code>=8)
      tab = &m_DCT3Dtab2[(code>>0)-8];
    else {
   ...
   if (tab->val==ESCAPE) { /* escape */
    ...
    qval = &bp[j];
    if (comp >= 6)
      QP = mmax (1, mmin( 31, (m_bquant_tab[m_bquant] * m_quant ) >> 2 ));
    else 
      QP = m_quant;
...
void CH263Dec::get_sac_block(int comp, int ptype)
{
  int position=0;
  ...
  while (!last) {	/* while there are DCT coefficients remaining */
    ...
    if (TCOEF_index == ESCAPE_INDEX) {        /* ESCAPE code encountered */
      ...
      symbol_word = m_tcoeftab[TCOEF_index];
...
CH263Dec::RunCoef CH263Dec::vlc_word_decode(int symbol_word, int *last)
{
  int sign_index;
  RunCoef DCTcoef;
  ...
  *last = (symbol_word >> 12) & 01;
  ...
  sign_index = decode_a_symbol(m_cumf_SIGN);	

  DCTcoef.sign = m_signtab[sign_index];
...
CH263Dec::RunCoef CH263Dec::Decode_Escape_Char(int intra, int *last)
{
  int last_index, run, run_index, level, level_index;
  RunCoef DCTcoef;

  if (intra) {
    last_index = decode_a_symbol(m_cumf_LAST_intra);
    *last = m_last_intratab[last_index];
  }
  else {
    last_index = decode_a_symbol(m_cumf_LAST);
    *last = m_lasttab[last_index];
  }

  if (intra) 
    run_index = decode_a_symbol(m_cumf_RUN_intra);
  else
    run_index = decode_a_symbol(m_cumf_RUN);

  run = m_runtab[run_index];

  DCTcoef.run = run;

  if (intra)
    level_index = decode_a_symbol(m_cumf_LEVEL_intra);
  else
    level_index = decode_a_symbol(m_cumf_LEVEL);

  level = m_leveltab[level_index];
...
int CH263Dec::DecodeTCoef(int position, int intra)
{
  int index;
  ...
  case 1:
    {
      if (intra) 
        index = decode_a_symbol(m_cumf_TCOEF1_intra);
      else 
        index = decode_a_symbol(m_cumf_TCOEF1); 
      break; 
    }
  case 2:
    {
      if (intra) 
        index = decode_a_symbol(m_cumf_TCOEF2_intra);
      else
        index = decode_a_symbol(m_cumf_TCOEF2);
      break; 
    }
  case 3:
    {
      if (intra) 
        index = decode_a_symbol(m_cumf_TCOEF3_intra);
      else
        index = decode_a_symbol(m_cumf_TCOEF3);
      break; 
    }
  default: 
    {
      if (intra) 
        index = decode_a_symbol(m_cumf_TCOEFr_intra);
      else
        index = decode_a_symbol(m_cumf_TCOEFr);
      break; 
    }
  }
