...
if (isset($_GET['year']) && isset($_GET['month']) && isset($_GET['day']) && isset($_GET['filenamenoext'])) {
	...
	$filenoextnothumb = $_GET['year']."/".$_GET['month']."/".$_GET['day']."/".str_replace("_thumb", "", $_GET['filenamenoext']);
	if ($doc_root == "") {
		$filenoext =        "$filesdirpath/$filenoext";
		$filenoextnothumb = "$filesdirpath/$filenoextnothumb";
	}
	else {
		$filenoext =        rtrim($doc_root,"\\/")."/".ltrim($filesdirpath,"\\/")."/$filenoext";
		$filenoextnothumb = rtrim($doc_root,"\\/")."/".ltrim($filesdirpath,"\\/")."/$filenoextnothumb";
	}
	...
	if (is_file("$filenoextnothumb.jpg"))
		strong_rename("$filenoextnothumb.jpg","$filenoextnothumb.jpg.recycled");
