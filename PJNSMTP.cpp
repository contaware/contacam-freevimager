...
*/
BOOL CPJNSMTPConnection::SendBodyPart(CPJNSMTPBodyPart* pBodyPart, BOOL bRoot, DWORD dwSendBufferSize/*=4096*/)
{
	BOOL bContinue = TRUE;
	...
	if (bContinue)
	{
		...
		{
			CPJNSMTPBodyPart* pChildBodyPart = pBodyPart->GetChildBodyPart(i);
			bContinue = SendBodyPart(pChildBodyPart, FALSE);
		}
...
*/
void CPJNSMTPConnection::SendMessage(CPJNSMTPMessage& Message, DWORD dwSendBufferSize/*=4096*/)
{
	USES_CONVERSION;
	...
	{
		//Send the root body part (and all its children)
		bContinue = SendBodyPart(&Message.m_RootPart, TRUE);
	}
	else
	{
		CString sBody = Message.m_RootPart.GetText();
