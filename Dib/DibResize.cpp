...
BOOL CDib::LanczosResampleBits(	DWORD dwNewWidth,
								DWORD dwNewHeight,
								CDib* pSrcDib/*=NULL*/,
								CWnd* pProgressWnd/*=NULL*/,
								BOOL bProgressSend/*=TRUE*/,
...
{
	// Radius
	const double dRadius = 6.0;
	const int nRadiusStart = -5;
	const int nRadiusEnd = 7;
	const int n2Radius = 12;

	/* Really Slow
	const double dRadius = 12.0;
	const int nRadiusStart = -11;
	const int nRadiusEnd = 13;
	const int n2Radius = 24;
	*/
	...
	if (pSrcDib->GetBitCount() <= 8)
	{
		...
		double yScale = (double)dwSrcHeight / (double)dwNewHeight;
		double f_x, f_y, a, b, rr, gg, bb, r1, r2;
		double kernelx[n2Radius]; // precalculated kernel values
		int i_x, i_y, xx, yy;
		COLORREF crColor;
		...
		for (unsigned int y = 0 ; y < dwNewHeight ; y++)
		{
			...
			for (unsigned int x = 0 ; x < dwNewWidth ; x++)
			{
				...
				for (int n = nRadiusStart ; n < nRadiusEnd; n++)
					kernelx[n - nRadiusStart] = KernelLanczosSinc((double)n - b, dRadius);
				
				rr = gg = bb = 0.0;
				
				for (int m = nRadiusStart ; m < nRadiusEnd ; m++)
				{
					r1 = KernelLanczosSinc((double)m - a, dRadius);
					yy = i_y + m - 1; // Clamp done by GetPixelColor()
					for (int n = nRadiusStart ; n < nRadiusEnd ; n++)
					{
						r2 = r1 * kernelx[n - nRadiusStart];
						xx = i_x + n - 1; // Clamp done by GetPixelColor()
						crColor = pSrcDib->GetPixelColor(xx, yy);
						rr += (GetRValue(crColor) * r2);
						gg += (GetGValue(crColor) * r2);
	...
	else if (pSrcDib->HasAlpha() && pSrcDib->GetBitCount() == 32)
	{
			double xScale = (double)dwSrcWidth  / (double)dwNewWidth;
		double yScale = (double)dwSrcHeight / (double)dwNewHeight;
		double f_x, f_y, a, b, rr, gg, bb, aa, r1, r2;
		double kernelx[n2Radius]; // precalculated kernel values
		int i_x, i_y, xx, yy;
		COLORREF crColor;
			...
			for (unsigned int x = 0 ; x < dwNewWidth ; x++)
			{
				...
				for (int m = nRadiusStart ; m < nRadiusEnd ; m++)
				{
					...
					for (int n = nRadiusStart ; n < nRadiusEnd ; n++)
					{
						r2 = r1 * kernelx[n - nRadiusStart];
						xx = i_x + n - 1; // Clamp done by GetPixelColor()
						crColor = pSrcDib->GetPixelColor32Alpha(xx, yy);
						rr += (GetRValue(crColor) * r2);
						gg += (GetGValue(crColor) * r2);
						bb += (GetBValue(crColor) * r2);
						aa += (GetAValue(crColor) * r2);
					}
				}
				...
				if (aa < 0.0) aa = 0.0;
				else if (aa > 255.0) aa = 255.0;
				SetPixelColor32Alpha(x, y, RGBA(Round(rr), Round(gg), Round(bb), Round(aa)));
			}
		}
