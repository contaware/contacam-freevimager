...
InstallCheckInstallerRunning:
  ...
  MessageBox MB_OK|MB_ICONEXCLAMATION "L'installer sta già andando" /SD IDOK
  ClearErrors
  Pop $R1
  Pop $R0
  ${UAC.Unload} ;Must call unload!
...
!ifdef INSTALLER_NT
    Delete "$INSTDIR\${APPNAME_NOEXT}wIta.exe"
!endif
    goto unicode_end
unicode:
!ifdef INSTALLER_WIN9X & INSTALLER_NT
...
!endif
!ifdef INSTALLER_NT
    Rename "$INSTDIR\${APPNAME_NOEXT}wIta.exe" "$INSTDIR\${APPNAME_EXT}"
!endif
unicode_end:
...
VIAddVersionKey /LANG=${LANG_ITALIAN} "ProductName" "${APPNAME_NOEXT} Application"
VIAddVersionKey /LANG=${LANG_ITALIAN} "Comments" "MM Application"
VIAddVersionKey /LANG=${LANG_ITALIAN} "CompanyName" "Contaware.com"
VIAddVersionKey /LANG=${LANG_ITALIAN} "LegalTrademarks" "${APPNAME_NOEXT} is a trademark of Contaware.com"
VIAddVersionKey /LANG=${LANG_ITALIAN} "LegalCopyright" "© Contaware.com"
VIAddVersionKey /LANG=${LANG_ITALIAN} "FileDescription" "Installation Routine of ${APPNAME_NOEXT}"
VIAddVersionKey /LANG=${LANG_ITALIAN} "FileVersion" "${APPVERSION}.0"
...
UninstallCheckUninstallerRunning:
  ...
  MessageBox MB_OK|MB_ICONEXCLAMATION "L'uninstaller sta già andando" /SD IDOK
  ClearErrors
  Pop $R1
  Pop $R0
  ${UAC.Unload} ;Must call unload!
