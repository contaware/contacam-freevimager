...
// Return type		: bool 
// Argument         : LPCTSTR lpszFullPath
//		file/folder to be added
// Argument         : bool bStorePath = true
//		flag which says whether to store the entire file/folder path or only the file/folder name
// Argument         : int iLevel = Z_DEFAULT_COMPRESSION
//		the compression level (see OpenNewFile() for detailed description)
// Argument         : DWORD nBufSize = 65535
//		the size of the buffer used during compression
	bool AddNewFile(LPCTSTR lpszFullPath, bool bStorePath = true, int iLevel = Z_DEFAULT_COMPRESSION, unsigned long nBufSize = 65535);
...
// Argument         : LPCTSTR lpszFullPath
//		file/folder to be added
// Argument         : LPCTSTR lpszStoredPath
//		file/folder stored path 
// Argument         : int iLevel = Z_DEFAULT_COMPRESSION
//		the compression level (see OpenNewFile() for detailed description)
// Argument         : DWORD nBufSize = 65535
//		the size of the buffer used during compression
	bool AddNewFile(LPCTSTR lpszFullPath, LPCTSTR lpszStoredPath, int iLevel = Z_DEFAULT_COMPRESSION, unsigned long nBufSize = 65535);
...
//		structure that have additional information; the following fields are valid:
//			- m_uMethod - file compression method; can be 0 (storing) or Z_DEFLATE (deflating)
//				otherwise Z_DEFLATE is assumed
//			- m_uModDate, m_uModTime - use SetTime method of CFileHeader to set them
...
//		the path to the file to retreive date and attributes from
	bool OpenNewFile(CFileHeader & header, int iLevel = Z_DEFAULT_COMPRESSION, LPCTSTR lpszFilePath = NULL);
...
// Function name	: MakeRenamedFileName
// Description	    : if necessary changes the name to make sure
//                  : that the file name is not existing.
//                  : changed names will have numbers appended.
// Return type		: the new file name 
	CString MakeRenamedFileName(LPCTSTR lpszFileName);
...
// Argument         : bool bFailIfFileExists = false
//		fail if file (not directory) exists
// Argument         : bool bRenameIfFileExists = false
//		rename file (not directory) if it exists
// Argument         : DWORD nBufSize = 65535
//		the size of the buffer used during extraction
	bool ExtractFile(	WORD uIndex,
						LPCTSTR lpszPath,
						bool bFailIfFileExists = false,
						bool bRenameIfFileExists = false,
...
// Function name	: FileOnlyExits
// Description	    : check whether the given file exists
// Return type		: static bool 
//		return true if the file exists
//		return false if the file is not existing
	static bool FileOnlyExits(LPCTSTR lpszFileName);
