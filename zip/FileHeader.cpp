...
bool CFileHeader::Read(CZipStorage *pStorage)
{
	...
	char* pName = (char*)new char[m_uFileNameSize+1];
	pStorage->m_file.Read(pName, m_uFileNameSize);
	pName[m_uFileNameSize] = '\0'; //  Make Sure it is Null Terminated!
	m_szFileName = CString(pName);
	delete [] pName;
	...
	if (m_uCommentSize)
	{
		char* pComment = (char*)new char[m_uCommentSize+1];
		pStorage->m_file.Read(pComment, m_uCommentSize);
		pComment[m_uCommentSize] = '\0'; //  Make Sure it is Null Terminated!
		m_szComment = CString(pComment);
		delete [] pComment;
	}
...
{
#ifdef _UNICODE
	char* filename = new char[2*m_szFileName.GetLength() + 1];
	if (filename)
	{
		...
		if (size == -1)
		{
			...
			if (filename)
				m_uFileNameSize = (WORD)::wcstombs(filename, (LPCTSTR)m_szFileName, 2*m_szFileName.GetLength() + 1);
		}
		else
			m_uFileNameSize = (WORD)size;
	}
#endif
	...
	if (filename)
	{
		memcpy(buf + 46, filename, m_uFileNameSize);
		delete [] filename;
	}
#else
	...
	{
#ifdef _UNICODE
		char* comment = new char[2*m_uCommentSize + 1];
		if (comment)
		{
			if (::wcstombs(comment, (LPCTSTR)m_szComment, 2*m_uCommentSize + 1) == -1)
				memset(comment, '?', m_uCommentSize);
			memcpy(buf + 46 + m_uFileNameSize + m_uExtraFieldSize, comment, m_uCommentSize);
			delete [] comment;
		}
#else
...
bool CFileHeader::ReadLocal(CZipStorage *pStorage, WORD& iLocExtrFieldSize, bool& bIsDataDescr)
{
	...
	// Give the priority to the local flag
	WORD uTemp = *((WORD*)(buf + 6));
	if ((uTemp & 0xf) != (m_uFlag & 0xf))
		m_uFlag = uTemp;

	// Data descriptor present
	bIsDataDescr = (uTemp & 8) != 0;
	...
	// uLocalFileNameSize may be different from m_uFileNameSize
	// (it may contain the disk name for example)
	WORD uLocalFileNameSize = *((WORD*)(buf + 26)); 
	...
	pStorage->m_file.Seek(uLocalFileNameSize, CFile::current);
...
void CFileHeader::WriteLocal(CZipStorage& storage)
{
	...
	if (filename)
	{
		memcpy(buf + 30, filename, m_uFileNameSize);
		delete [] filename;
	}
#else
