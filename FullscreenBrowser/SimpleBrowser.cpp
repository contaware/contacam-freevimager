						   ...
						   UINT nID)
{
	...
	if (_Browser != NULL) {
		...
		HRESULT			hr       = S_OK;
		int nCountDown           = 30;
		while ((document == NULL) && (hr == S_OK) && nCountDown-- > 0) {

			::Sleep(100);

			IDispatch *document_dispatch = NULL;

			hr = _Browser->get_Document(&document_dispatch);	// nCountDown is necessary because on Win9x this returns S_OK
																// with document_dispatch being NULL...
