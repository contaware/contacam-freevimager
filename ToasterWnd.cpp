...
BEGIN_MESSAGE_MAP(CHtmlLayoutReflectorWnd, CWnd)
  ON_MESSAGE(WM_BEHAVIOR_NOTIFY, &CHtmlLayoutReflectorWnd::OnBehaviorNotify)
END_MESSAGE_MAP()
...
BOOL CToasterWnd::UpdateTransparency(BYTE byAlpha)
{
  UNREFERENCED_PARAMETER(byAlpha); //Just in case we do not use it in this method
...
{
  //What will be the return value from this method (assume the worst)
  HRESULT hr = MAKE_HRESULT(SEVERITY_ERROR, FACILITY_WIN32, ERROR_NOT_SUPPORTED);

__if_exists(CImage)
{
  hr = m_Logo.Load(pszFilename);
  if (SUCCEEDED(hr))
  {
...
{
  UNREFERENCED_PARAMETER(pDC); //Just in case we do not use it in this method

__if_exists(CImage::AlphaBlend)
{
...
{
  BOOL bAppThemed = TRUE;

__if_exists(CWnd::IsAppThemed)
{
