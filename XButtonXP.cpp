...
LRESULT CXButtonXP::OnMouseHover(WPARAM wparam, LPARAM lparam) 
{
	wparam;
	lparam;
...
void CXButtonXP::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
	...
	if (IsThemed())
	{
		...
		if (ThemeHelper.IsThemeBackgroundPartiallyTransparent(m_hTheme, part, state))
			ThemeHelper.DrawThemeParentBackground(GetSafeHwnd(), memDC.m_hDC, &itemRect);
	...
	if (bHasText)
	{
		...
		if (IsThemed())
		{
			ThemeHelper.DrawThemeText(m_hTheme, memDC.m_hDC, part, bIsDisabled ? PBS_DISABLED : PBS_NORMAL,
