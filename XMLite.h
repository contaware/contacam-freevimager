...
typedef struct _tagParseInfo
{
	...
	LPXDoc		doc;
	_tagParseInfo() {	trim_value = false;
						entity_value = true;
						force_parse = false;
...
typedef struct _tagDispOption
{
	...
	TCHAR newline_type[3];		// may be: _T("\n") , _T("\r") or _T("\r\n") 
	...
	int tab_base;				// internal usage for tabs
	_tagDispOption() {	newline = true;
						_tcsncpy(newline_type, _T("\r\n"), 3);
						reference_value = true;
						entitys = &entityDefault;
						tab_base = 0;
						value_quotation_mark = _T('"'); }
}DISP_OPT, *LPDISP_OPT;
...
typedef struct _tagXMLNode
{
	...
	// Either Child(s) or Value
	void ClearValueIfHasChild();
	
	// Clear Space Only Values
	void ResetSpaceOnlyValues(const CString& sResetValue = _T(""));
	...
	LPCTSTR	GetAttrName( LPCTSTR attrvalue );
	...
	// search node
	LPXNode	Find( LPCTSTR name , BOOL bOnlyDirectChilds = FALSE);
	LPXNode	Find( LPCTSTR name, LPCTSTR attrname, BOOL bOnlyDirectChilds = FALSE );
	LPXNode	Find( LPCTSTR name, LPCTSTR attrname, LPCTSTR attrvalue, BOOL bOnlyDirectChilds = FALSE );
	LPXNode	FindByAttrName( LPCTSTR name, LPCTSTR attrname, BOOL bOnlyDirectChilds = FALSE );
	LPXNode	FindByAttrValue( LPCTSTR name, LPCTSTR attrvalue, BOOL bOnlyDirectChilds = FALSE );
	LPXNode	FindByAttrName( LPCTSTR attrname, BOOL bOnlyDirectChilds = FALSE );
	LPXNode	FindByAttrValue( LPCTSTR attrvalue, BOOL bOnlyDirectChilds = FALSE );
...
// Helper Funtion
__forceinline long XStr2Int( LPCTSTR str, long default_value = 0 )
{
	return ( str && *str ) ? _ttol(str) : default_value;
}

__forceinline bool XIsEmptyString( LPCTSTR str )
{
	CString s(str);
	s.TrimLeft();
	s.TrimRight();
